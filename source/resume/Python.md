title: "简历"
date: 2015-04-01 08:45:24
---
## <center>求职简历 </center>
#### 基本信息
姓名： 李强  
电话： 15580018851  
微信：csrgxtu  
邮箱： 1246506786@qq.com  
Github: https://github.com/csrgxtu  
Blog： http://csrgxtu.github.io/  
出生日期： 1991-06-26

学历： 学士(湘潭大学），硕士（湘潭大学）  
本科专业： 信息与计算科学  
研究生专业及研究方向： 计算数学，专利文献信息检索，基于统计方法的篮球胜负分析预测， 数据挖掘，搜索引擎


*****

#### 总结
我之前的主要方向是后台系开发和大数据处理．因为工作学习的缘故，接触了Linux下系统，网络编程，系统架构，搜索引擎，自然语言处理，书籍图片处理，推荐系统等相关知识．

可以熟练使用Python，C，Golang，NodeJs，Java开发．喜欢阅读，实践新的技术，方法．

十分喜欢Python的哲学，想继续朝着基础后台开发方向深入研究.
*****


#### 技能
* 后台系统，Linux系统编程，socket编程，数据库，微服务，系统运维，大数据处理，聚类，分类
* Python，C，Golang，NodeJs，Java，Lua，Html
* Flask，Beego，Express，SailsJs，SpringBoot，Bootstrap，JQuery
* Nginx，Docker，ElasticSearch，Apache，Freeswitch，MQTT
* Spark，MySQL，PostgreSQL，Hadoop，Hive，MongoDB，Redis
* Git，SVN，Github，DockerHub
* 基本数学，统计学知识
* 英语四六级
*****

#### 工作经历
** 大数据开发工程师 **  
深圳数位传媒科技有限公司（离职）  
2016-12 2017-2  
广东 深圳 南山区 软件基地5C  
* 基础POI数据标注
* 实时计算模块开发

**后台开发，系统架构方向**  
广州美读信息科技有限公司 　
2015-09 2016-12  
广东 广州 天河区 华强路 保利克洛维
* 美丽阅读书籍推荐系统，大数据处理(Python)
* 豆瓣，亚马逊，晒书房书籍数据的分布式爬虫系统(Python)
* 使用OpenCV，OCR提高书架扫书的正确率(Python)
* 使用Hadoop, Hive分析生产日志，统计信息(Python)
* 使用Golang开发美读支付平台（基于微信，支付宝）
* 美丽阅读公众号功能全栈开发(Go)
* 后台整体架构重构（API Gateway）
* 添加语音书籍搜索功能(demo)
* 书架项目后台开发(接手)
* 给孩子的诗后台开发
* 2016年七夕以书识人系统开发（６小时10Ｗ访问）
* 秘密书架运营后台的全栈开发(Go)
* 运营部门KPI指标展板全栈开发(Ruby)
* 一些琐碎的事情，如数据分析etc

收获：
* 加强了Python, NodeJs的开发经验
* 熟练使用Golang开发系统
* 熟悉API Gateway
* 熟悉身份认证系统
* 意识到扎实的基础工程能力的重要性
* 公司随处可见的书籍资料使我感觉充实

**后台开发工程师**  
湖南亿邦通信有限责任公司  
2014-02 -- 2015-08  
湖南 长沙 岳麓区 雷锋大道 335号

* 使用C语言开发从RTSP到RTMP视频流转发功能
* 在安霸 海斯等IP摄像头上做嵌入式C开发
* 负责配置管理FreeSwitch, MQTT和Nginx服务器  
* 主要负责使用NodeJs和SailsJs框架开发后台，向前端开发人员提供RESTful接口
* 使用Java给Android开发文本资料检索系统
* 使用Java的SSH框架开发湖南省体育局视频信息系统
* 使用Docker技术加快项目部署和维护

收获：
* 熟练使用NodeJs，了解其编程的思想。
* 第一次在路由器上使用C开发流媒体协议相关的程序
* 学会在固定产品摄像头上做嵌入式开发
* 熟悉使用实时消息的推送和接收
* 接触诸多知识，如Android开发，前端开发，Lua开发等

#### 在校经历
**研究生阶段科研简介**  
专利文献信息检索  
基于统计方法的篮球胜负分析预测  
2014-06 -- 2016-06  
中国 湖南 湘潭大学 数学与计算科学学院

* 使用Python，Java采集NBA篮球比赛数据
* 调研相关算法的编程语言实现，如极大熵，HMM等(Python)
* 学习搜索引擎基本原理，并实现简单的搜索引擎
* 使用Lucene对大量的专利文本建立索引
* 调研相关统计模型，理解其应用背景
* 根据实际采集的数据，构建模型训练并预测(Python)

收获：

* 熟悉Lucene的使用方法
* 了解分词，倒排索引，词性标注的原理
* 理解了搜索引擎的原理，并实现自己的搜索引擎原型 [Google](https://github.com/csrgxtu/google)
* 熟悉了基本的分类聚类算法及一些稍微复杂点的分类算法
* 熟悉从数据整理到模型构建到预测的大数据处理流程
* 熟练使用Python， Java， Linux编程环境



**服务器运维工程师**  
湘潭大学 网络信息管理中心  
2013-08 至 2014-01  
中国 湖南 湘潭大学 网络中心

在湘潭大学网络中心，负责维护学校主页服务器及各个院系服务器，使用Bash,Perl, Python自动化服务器管理。同时涉及服务器硬件的组装维修。

**语音识别科研项目助理**  
湘潭大学 数学院  
2012-07 至 2012-10  
中国 湖南 湘潭大学 数学院

在由黄山老师领导的科研项目 -- *基于大词汇量连续语音识别* 项目中，负责Linux服务器环境维护，使用Perl， Bash进行分词处理。了解语音处理中的分词，标注，HMM模型，HTK工具集。

*****

#### Projects
**[GoogleHosts-Python](https://github.com/csrgxtu/GoogleHosts-Python)**
因为科研中经常使用Google，但是中国屏蔽了Google的服务，所以就开发出这个Project，动态检索计算出优质的IP，以使我们能无障碍访问Google的服务。

**[shampo](https://github.com/csrgxtu/shampoo)**
使用Android播放rtsp流。

**[Shepherd](https://github.com/csrgxtu/Shepherd)**
使用Golang实现的GPS追踪的服务端。


**[Google Python Search](https://github.com/csrgxtu/gps)**
自己为了无障碍访问Google的搜索服务，在公网服务器上架设的一个Google镜像搜索。

**[maxent](https://github.com/csrgxtu/maxent)**
使用极大熵做篮球预测

**[Googlebot](https://github.com/csrgxtu/Googlebot)**  
一个多线程的网页爬虫，可以无限制抓取网页信息。

**[Google](https://github.com/csrgxtu/Google)**
因为学习搜索引擎的缘故，自己实现的一个简单的搜索引擎，能自动抓取网页，然后检索，然后提供搜索功能。

*****

<center>Powered By Markdown</center>

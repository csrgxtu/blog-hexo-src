...
但是有时候，我们确实需要在一个域名中通过XMLHttpRequest访问另一个域名的资源。所以就有了**CORS**技术。一般情况下，各个浏览器实现XMLHttpRequest的时候已经支持了CORS功能，所以只要在服务端开启CORS，即可以跨域资源共享了。

要开通CORS，服务器端的HTTP头部只需要添加*Access-Control-Allow-Origin: example.com*即可。客户端在请求是头部添加*Origin: http://example.com*即可。下面是一个CORS请求的头部信息：
```bash
请求
